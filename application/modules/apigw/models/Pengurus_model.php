...
class Pengurus_model extends CI_Model{
    ...
    function insertdata_pengurus($dt){
        $sql_hdr = $this->db->insert_string('tbl_pengurus_lembaga', $dt);
   ...
   public  function get_list_approval_pengurus(){
        $this->db->select('P.id_pengurus, P.id_koperasi, K.nm_koperasi, K.no_badan_hukum, P.`status`');
        $this->db->from('tbl_koperasi K');
        $this->db->join('tbl_pengurus_lembaga P', 'ON K.id_koperasi = P.id_koperasi','inner');
